...
class IMUJointAngleCalculator:
    ...
    def calculate_orientation(self, sensor_num, calibration_period=1.0, alpha=0.98):
        """
        ...
        Returns:
            np.array: Array of quaternions representing orientation over time
        """
    ...
    def calculate_joint_angle(self, sensor1, sensor2, calibration_period=1.0, 
                            alpha=0.98, angle_type='relative'):
        """
        Calculate joint angle between two sensors.
        ...
        """
        # Get orientations for both sensors
        quat1 = self.calculate_orientation(sensor1, calibration_period, alpha)
        quat2 = self.calculate_orientation(sensor2, calibration_period, alpha)
    ...
    def calculate_joint_angle_3d(self, sensor1, sensor2, calibration_period=1.0, alpha=0.98):
        """
        Calculate 3D joint angles (roll, pitch, yaw) between two sensors using gyroscope and accelerometer data.
        Uses a complementary filter to combine gyro and accel data.
        ...
        joint_angles_rad = angles2 - angles1
    ...
    def plot_joint_angle(self, angle_data, angle_data_3d, title="Joint Angle"):
        ...
        Args:
            angle_data (dict): Result from calculate_joint_angle
        ...
        plt.title('3D Joint Angles Between Sensors')
        plt.legend()
        plt.grid(True)
        plt.show()
    ...
    def save_joint_angles_to_csv(self, angle_data, angle_data_3d, file_path, joint_name="joint"):
        ...
        Args:
            angle_data (dict): Result from calculate_joint_angle (relative or Euler angles)
            angle_data_3d (dict): Result from calculate_joint_angle_3d (3D angles)
...
if __name__ == "__main__":
    ...
    calculator = IMUJointAngleCalculator("H:/Tharinda_Walking_Pattern_02_20250720_084612.xlsx")
    
    # Define which sensors to use (these are example numbers - adjust based on your data)
    sensor1 = 6 
    sensor2 = 8  
    
    # Calculate joint angle with:
    # - 1.0 second calibration period at start
    # - Complementary filter alpha = 0.98 (98% gyro, 2% acc)
    # - 'relative' angle type (can also use 'euler' for separate angles)
    angle_data = calculator.calculate_joint_angle(
        sensor1, sensor2, 
    ...
    angle_data_3d = calculator.calculate_joint_angle_3d(
        sensor1, sensor2,
    ...
    # Plot the results
    calculator.plot_joint_angle(angle_data, angle_data_3d, title=f"Joint Angle between Sensor {sensor1} and {sensor2}")

    # Save to CSV with custom joint name
    calculator.save_joint_angles_to_csv(
        angle_data,
        angle_data_3d,
        file_path="H:/Tharida_Walking_Patter_01.csv",
